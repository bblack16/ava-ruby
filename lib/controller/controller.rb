...
module Ava
  class Controller
    ...
    RESERVED_NAMES = [:controller, :methods, :addresses].freeze
    ...
    def handle_request(client)
      ...
      msg = decrypt(remote_ip, YAML.load(client.recv(1_000_000_000)))
      return msg.to_yaml if msg[:status] == 404
      response = if msg[:secret_key]
                   { status: 202, response: register_client(remote_ip, msg[:secret_key]) }
                 elsif validate_connection(remote_ip, msg)
                   run_command(msg)
                 else
                   { status: 401, error: ArgumentError.new('Invalid or missing client ID.') }
                 end
